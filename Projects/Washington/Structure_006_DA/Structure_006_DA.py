...
#    test_set_path='../Sets/cv1/test.h5'
    ...
    num_epochs_before_validation = 2
    ...
    validSet, valid_imageHeight, valid_imageWidth, valid_labels = load_dataset(validation_set_path,DataBasePath, log_file_indicator, x_pad = 50, y_pad = 50, database = 'Washington')
    if not testFLAG:
       if DataAugmentation:
          padding = False
       else:
          padding = True
       trainSet, train_imageHeight, train_imageWidth, train_labels = load_dataset(train_set_path,DataBasePath, log_file_indicator, x_pad = 50, y_pad = 50,  padding = padding, database = 'Washington')
       ...
       testSet,  test_imageHeight, test_imageWidth, test_labels = load_dataset(test_set_path,DataBasePath, log_file_indicator, x_pad = 50, y_pad = 50, database = 'Washington')
    ...
    if not testFLAG:
        for step in range(ceil(restore_checkpoint_at_epoch/num_epochs_before_validation), num_steps):
           ...
           train(step, network_train, num_epochs_before_validation, batch_size, learning_rate, trainSet, imageHeight, train_imageWidth, num_classes, log_file_indicator, models_path, train_writer, transferFLAG, DataAugmentation)
           ...
           validation(epoch, network_train, batch_size,'train', trainSet,imageHeight,train_imageWidth,labels, num_classes,log_file_indicator, models_path, valid_ts_writer, dataAugmentation = DataAugmentation)
